...
public class ImageComparison extends BaseKeyword{
	 ...
	 */
	@Keyword
	static boolean areMatched(String expectedImgPath, String actualImgPath, String savePath, FailureHandling flowControl = FailureHandling.STOP_ON_FAILURE){
		try{
			...
			if(diff.hasDiff()){
				...
				String outputFilename = filename.substring(0, filename.lastIndexOf(".")) + "_diff." + extension
				ImageIO.write(diff.getMarkedImage(), extension, Paths.get(savePath).resolve(outputFilename).toFile())
